...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      localStorage.setItem("vrent_is_admin", isAdminValue.toString());
      localStorage.setItem("vrent_login_email", form.email);

      // Debug log เพื่อตรวจสอบข้อมูลที่เก็บใน localStorage
      console.log("LocalStorage Debug:", {
        vrent_user_id: data.full_name || form.email,
        vrent_full_name: data.full_name || form.email,
        vrent_is_admin: isAdminValue.toString(),
        vrent_login_email: form.email,
      });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-black to-slate-800 text-white overflow-hidden relative">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div
          className="absolute -bottom-40 -left-40 w-96 h-96 bg-gradient-to-br from-blue-400/30 to-purple-500/30 rounded-full blur-3xl animate-pulse"
          style={{ animationDelay: "1s" }}
        />
        <div
          className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-80 h-80 bg-gradient-to-br from-emerald-400/20 to-cyan-500/20 rounded-full blur-3xl animate-pulse"
          style={{ animationDelay: "2s" }}
        />

        {/* Additional floating gradients */}
        <div
          className="absolute top-20 left-20 w-32 h-32 bg-gradient-to-br from-pink-400/20 to-rose-500/20 rounded-full blur-2xl animate-pulse"
          style={{ animationDelay: "0.5s" }}
        />
        <div
          className="absolute bottom-20 right-20 w-40 h-40 bg-gradient-to-br from-indigo-400/20 to-blue-500/20 rounded-full blur-2xl animate-pulse"
              ...
              >
                ...
                <div className="group">
                  ...
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3 sm:pl-4 flex items-center pointer-events-none">
                      <svg
                        className="h-4 w-4 sm:h-5 sm:w-5 text-white/60 group-focus-within:text-yellow-400 transition-colors duration-300"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
                ...
                {error && (
                  <div className="rounded-xl sm:rounded-2xl border border-red-400/40 bg-gradient-to-r from-red-500/15 to-red-600/15 px-4 sm:px-5 py-3 sm:py-4 text-xs sm:text-sm text-red-200 flex items-center gap-2 sm:gap-3 animate-pulse shadow-lg shadow-red-500/20">
                    <svg
                      className="h-4 w-4 sm:h-5 sm:w-5 text-red-400 flex-shrink-0"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                ...
                <div className="flex items-center justify-between">
                  <label className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm text-white/85 hover:text-white transition-colors duration-300 cursor-pointer">
                    <input
                      type="checkbox"
                      className="rounded border-white/40 bg-white/10 text-yellow-400 focus:ring-yellow-400/40 w-3.5 h-3.5 sm:w-4 sm:h-4"
                    />
